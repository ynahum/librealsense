...
public:
    op_reader( std::shared_ptr< realdds::dds_topic > const & topic );
    op_reader( std::shared_ptr< realdds::dds_participant > const & participant, std::string const & topic_name )
        : op_reader( op_payload::create_topic( participant, topic_name ) )
    {
    }
