...
op_reader::op_reader( std::shared_ptr< realdds::dds_topic > const & topic )
...
std::string const & op_reader::name() const
{
    return _reader->topic()->get()->get_name();
}


void op_reader::wait_for_writers( int n_writers, std::chrono::seconds timeout )
{
...
void op_reader::wait_for_data( std::chrono::seconds timeout )
{
    std::unique_lock< std::mutex > lock( _data_mutex );
...
op_reader::data_t op_reader::read()
{
    wait_for_data();
    return pop_data();
}
...
op_reader::data_t op_reader::pop_data()
{
    std::lock_guard< std::mutex > lock( _data_mutex );
...
op_reader::data_t op_reader::read( std::chrono::seconds timeout )
{
...
void op_reader::on_subscription_matched( eprosima::fastdds::dds::SubscriptionMatchedStatus const & status )
{
...
void op_reader::on_data_available()
{
    ...
    {
        data_t data;
        if( ! op_payload::take_next( *_reader, &data.msg, &data.sample ) )
        ...
        LOG_DEBUG( name() << ".on_data_available @" << timestr( received.to_ns(), timestr::no_suffix )
                          << timestr( notify_time, received, timestr::no_suffix ) << timestr( realdds::now(), notify_time ) << " "
                          << data.msg._op );
        got_something = true;
        _on_data( data );
    }
